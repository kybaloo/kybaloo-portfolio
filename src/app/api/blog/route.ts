    ...
    if (!fs.existsSync(filePath)) {
      ...
      const sampleBlogPosts = [
        ...
        },
        {
          ...
          summary:
            "Explore advanced animation techniques using Framer Motion to create engaging user interfaces in your React applications.",
          content:
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!fs.existsSync(filePath)) {
      return NextResponse.json({ error: "Blog data file does not exist" }, { status: 404 });
    }
    ...
    blogPosts = blogPosts.filter((p: BlogPost) => p.id !== id);
