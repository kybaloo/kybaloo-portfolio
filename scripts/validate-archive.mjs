...
const checks = [
  ['projects', () => { const d = read('projects');
    if (!Array.isArray(d) || d.length !== 10) throw new Error(`10 projets attendus, ${d?.length}`);
    ...
    d.forEach((p, i) => {
      if (!hasFields(p, ['id', 'title'])) throw new Error(`projet #${i} : id ou title manquant/vide`);
    });
    return `${d.length} projets`; }],
  ['blog', () => { const d = read('blog');
    if (!Array.isArray(d) || d.length !== 5) throw new Error(`5 articles attendus, ${d?.length}`);
    d.forEach((b, i) => {
      if (!hasFields(b, ['title', 'slug', 'content'])) throw new Error(`article #${i} : title, slug ou content manquant/vide`);
    });
  ...
  ['services', () => { const d = read('services');
    ...
    d.forEach((s, i) => {
      if (!hasFields(s, ['title', 'description'])) throw new Error(`service #${i} : title ou description manquant/vide`);
    });
    return `${d.length} services`; }],
  ['skills', () => { const d = read('skills');
    if (!Array.isArray(d) || d.length < MIN_SKILL_CATEGORIES)
      throw new Error(`au moins ${MIN_SKILL_CATEGORIES} catégories attendues, ${d?.length ?? 0} trouvées`);
    d.forEach((c, i) => {
      if (!Array.isArray(c.items) || c.items.length === 0)
        throw new Error(`catégorie #${i} (${c.category ?? "?"}) sans items[] non vide`);
    });
    const total = d.reduce((n, c) => n + c.items.length, 0);
    if (total < MIN_SKILLS) throw new Error(`au moins ${MIN_SKILLS} compétences attendues, ${total} trouvées`);
    return `${d.length} catégories, ${total} compétences`; }],
  ['experiences', () => { const d = read('experiences');
    if (!Array.isArray(d) || d.length !== 3) throw new Error(`3 expériences attendues, ${d?.length}`);
    d.forEach((e, i) => {
  ...
  ['profile', () => { const d = read('profile');
    for (const l of ['fr', 'en']) {
      ...
      if (heroCount < MIN_HERO_FIELDS)
        throw new Error(`profile.${l}.hero : au moins ${MIN_HERO_FIELDS} champs non vides attendus, ${heroCount} trouvés`);
      const aboutCount = countNonEmpty(about);
      if (aboutCount < MIN_ABOUT_FIELDS)
        throw new Error(`profile.${l}.about : au moins ${MIN_ABOUT_FIELDS} champs non vides attendus, ${aboutCount} trouvés`);
    }
    return 'fr + en'; }],
];
